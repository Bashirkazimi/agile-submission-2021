...
if __name__ == "__main__":
	...
	train_gen = utils.MultiData(conf.train_dir, train_files, conf.batch_size, True, dim=conf.dim, num_classes=conf.num_classes)
	valid_gen = utils.MultiData(conf.valid_dir, valid_files, conf.batch_size, False, dim=conf.dim, num_classes=conf.num_classes)
	...
	checkpoint = ModelCheckpoint(conf.model_path, monitor='val_sparse_mean_iou', mode='max', save_best_only=True, verbose=1)
